...
function nextSlide () {
  for (let i = 0; i < slides.length; i++) {
    if (slides[slides.length - 1].classList.contains('activeSlide')) {
      slides[0].setAttribute('class', 'mainSlides animation animation--time animation__slideFromRightToLeft activeSlide');
      slides[slides.length - 1].setAttribute('class', 'mainSlides animation animation--time animation__slideFromZeroToLeft');
      setTimeout(function () { slides[slides.length - 1].setAttribute('class', 'mainSlides') }, time);
      break;
    }
    if (slides[i].classList.contains('activeSlide')) {
      slides[i + 1].setAttribute('class', 'mainSlides animation animation--time animation__slideFromRightToLeft activeSlide');
      slides[i].setAttribute('class', 'mainSlides animation animation--time animation__slideFromZeroToLeft');
